...
const sendConfirmationToken = async (data: IData) => {
  const msg = {
    ...
    html: `<h1>Olá ${data.username}</h1> 
            ...
            <a target="_blank" href='https://caf-web.vercel.app/signup/confirmation?token=${data.code}&role=${data.role}'> Seguir com meu cadastro </a>`,
  };
  sgMail
    .send(msg)
    .then(() => {
...
const sendCodeAccess = async (data: IData) => {
  const msg = {
    ...
    html: `
            ...
            <h5>Você não precisa decorar essa senha, ela estará disponível pra você aqui neste email e na nossa <strong><h4><a href='https://caf-web.vercel.app/' target="_blank">PLATAFORMA</a></h4></strong></h5>
